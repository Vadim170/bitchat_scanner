...
- Precise location is required on every supported API level. The manifest declares `BLUETOOTH_SCAN` without `neverForLocation` because detections are combined with the phone position; the Bluetooth stack therefore only delivers results while `ACCESS_FINE_LOCATION` is granted and Location Services are on (Android 12+), and on Android 8.0–11 the location grant is the classic BLE scan requirement. `PermissionUtils.requiredPermissionsFor()` is the single source of that matrix and is unit-tested.
- On Android 12+ (API 31+), `BLUETOOTH_SCAN` and `BLUETOOTH_CONNECT` are requested in addition. Coarse location is requested together with precise so the system dialog offers the precise option.
- Location Services off is a distinct coordinator state (`LOCATION_DISABLED`) checked before any registration, because Android drops results silently in that case. The main screen offers the system location settings and `onResume` retries the persisted session.
- A visible session uses the active callback with the same BitChat filters as the hidden registration; unfiltered scans are suspended by Android on screen-off and demoted after the platform scan timeout. When that session is hidden on Android 8.0+, the coordinator registers the filtered system-managed `PendingIntent`; it must not create an Android service or request background location.
...
- Some vendor stacks reject the zero-length service-data presence filter. The shared fallback retains service-UUID and solicitation filters, so service-data-only advertisements can be missed. If even the fallback is rejected, only the visible session may continue unfiltered; the `PendingIntent` registration reports an error instead.
- The `PendingIntent` registration does not survive a Bluetooth adapter restart or a reboot, and Android does not deliver adapter state changes to manifest receivers. The persisted session resumes on the next `onStart`.
...
- Map tile requests go to the configured osmdroid/OpenStreetMap tile provider; detection records stay in the app-local database, are not sent to an app backend, and are excluded from cloud backup (`allowBackup="false"`) and from Android 12+ device-to-device transfer (`dataExtractionRules`).
