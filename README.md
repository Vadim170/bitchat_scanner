...
- Detection history is stored in the app-local database; the app does not sync or upload records to an app backend. The database and preferences are excluded from cloud backup and from Android 12+ device-to-device transfer.
- The full privacy policy lives in [docs/PRIVACY_POLICY.md](docs/PRIVACY_POLICY.md); the Play Console steps are in [docs/GOOGLE_PLAY_CHECKLIST.md](docs/GOOGLE_PLAY_CHECKLIST.md).
...
- Precise location (`ACCESS_FINE_LOCATION`) is required on every supported version. The manifest declares `BLUETOOTH_SCAN` without `neverForLocation` because detections are tied to the phone position for the coverage map; as a consequence, Android 12+ only delivers scan results while precise location is granted and Location Services are on. On Android 8.0–11 (API 26–30) the location grant is the classic BLE scan requirement.
- `ACCESS_COARSE_LOCATION` is requested together with the precise permission so the Android 12+ dialog offers the precise option. Choosing "Approximate" is not sufficient for scan results.
- On Android 12+ (API 31+), `BLUETOOTH_SCAN` and `BLUETOOTH_CONNECT` are requested in addition.
- Coordinates are attached only to detections made while the app is visible; hidden detections never receive fresh coordinates.
...
- While the Activity is visible, results arrive through the active BLE scan callback. It uses the same BitChat service filters as the hidden registration: Android suspends unfiltered scans when the screen turns off and demotes them to opportunistic after its scan timeout (30 minutes by default), which would silently end a long visible session.
- If Location Services are off, the scanner reports a dedicated state and offers the system location settings, because Android drops BLE results for a location-attributed app without reporting an error.
...
- Some vendor BLE stacks reject the zero-length service-data presence filter. The fallback keeps service-UUID and solicitation filters, so service-data-only BitChat advertisements may be missed. If even the fallback is rejected, only the visible session continues unfiltered; the hidden registration never does.
- The hidden `PendingIntent` registration is lost when Bluetooth is turned off and on again or the device reboots. Android does not deliver adapter state changes to manifest receivers, so the persisted session resumes the next time the app is opened rather than automatically.
...
The **Android Release** workflow validates the `vX.Y.Z` format, requires the tagged commit to be an ancestor of `origin/main`, and requires the tag to match `versionName` exactly. To rerun an existing tag manually, choose **Actions → Android Release → Run workflow** and enter that existing tag in the required `tag` input (for example, `v1.1.0`); the workflow still validates the tag before accessing signing secrets.
...
To download the release package from Actions, open **Actions → Android Release**, select the completed tag run, choose **Artifacts**, and download `bitchat-scanner-v1.1.0-release-signed` (substitute the actual tag). From the extracted directory, verify `sha256sum -c bitchat-scanner-v1.1.0-release-signed.sha256`. For long-lived distribution, use the matching GitHub Release assets instead; use the R8 mapping artifact for crash analysis only.

For example, tag `v1.1.0` must match:
...
- Google Play submission checklist: [docs/GOOGLE_PLAY_CHECKLIST.md](docs/GOOGLE_PLAY_CHECKLIST.md)
